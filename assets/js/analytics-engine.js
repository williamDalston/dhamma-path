...
class AnalyticsEngine {
    ...
    monitorMemoryUsage() {
        if ('memory' in performance) {
            ...
            // Throttle memory usage tracking to every 7 seconds
            if (!this.lastMemoryTrack || performance.now() - this.lastMemoryTrack > 7000) {
                this.trackEvent('memory_usage', { 
                    used: this.metrics.system.memoryUsage,
                    total: memory.totalJSHeapSize / 1024 / 1024,
                    limit: memory.jsHeapSizeLimit / 1024 / 1024
                });
                this.lastMemoryTrack = performance.now();
            }
        }
    }
